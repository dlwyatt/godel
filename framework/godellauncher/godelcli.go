...
//
// <global flags> can be one of [--version], [--help], [--debug] or [--wrapper <path>]. Note that, unlike the behavior
// of some other CLI programs, the flags can only be specified exactly as described: for example, inputs of the form
// "--version=true", "--debug false" and "--wrapper=<path>" are not valid.
func ParseAppArgs(args []string) (GlobalConfig, error) {
	...
	if err != nil {
		return GlobalConfig{}, errors.Errorf("failed to determine working directory")
	}
	cfg.WorkingDir = wd
	...
	for len(remainingArgs) > 0 {
		...
		if strings.HasPrefix(currArg, "--") {
			...
			case "--wrapper":
				if len(remainingArgs) == 0 {
					return GlobalConfig{}, errors.Errorf("--wrapper flag must specify a value")
				}
				currArg = remainingArgs[0]
				remainingArgs = remainingArgs[1:]
				cfg.Wrapper = currArg
			default:
				return GlobalConfig{}, errors.Errorf("unrecognized flag: %s", currArg)
			}
			continue
		}
...
func TaskForInput(global GlobalConfig, tasks []Task) (Task, error) {
	if global.Task == "" {
		if !global.Help && global.Version {
			vTask := VersionTask()
			os.Args = append([]string{global.Executable}, vTask.Name)
			return vTask, nil
		}
		return helpFlagTask(tasks, globalFlags()), nil
	}

	tasksMap := make(map[string]Task)
	for _, t := range tasks {
		if _, ok := tasksMap[t.Name]; ok {
			return Task{}, fmt.Errorf("task %s is defined multiple times", t.Name)
		}
		tasksMap[t.Name] = t
	}
	...
	if !ok {
		return Task{}, errors.Errorf("unknown task: %s", global.Task)
	}
	return task, nil
}

func globalFlags() []nameWithDesc {
	versionTask := VersionTask()
	helpFlagTask := helpFlagTask(nil, nil)
	return []nameWithDesc{
		{
			name: "--" + versionTask.Name,
			desc: versionTask.Description,
		},
		{
			name: helpFlagTask.Name,
			desc: helpFlagTask.Description,
		},
		{
			name: "--debug",
			desc: "Run in debug mode (print full stack traces on failures and include other debugging output)",
		},
		{
			name: "--wrapper",
			desc: "path to the wrapper script for this invocation",
		},
	}
}
